...
def main():
    ...
    for base, syms, relocs, blob in files:
        ...
        reloc_owners = set()  # slice-filled arrays that carry reloc slots
        for item in slices:
            ...
            if usym in canon:
                ...
                # Pointer-bearing arrays (reloc slots present) are the ones
                # whose raw slice data can carry baked 0x08xxxxxx addresses
                # (e.g. the tail half of SIRO pointer tables). Pure data blobs
                # with no relocs are left alone so their 0x08xxxxxx-looking
                # bytes are never rewritten.
                if slots_by_owner.get(canon[usym]):
                    reloc_owners.add(cname)
            else:
        ...
        if manifest:
            ...
            all_reloc_owners.update(reloc_owners)
    if all_manifest:
        ...
        # Every 4-aligned word inside a runtime slice region of a
        # pointer-bearing array is a candidate baked 0x08xxxxxx GBA address
        # (e.g. SIRO pointer tables whose tail is raw .incbin data, not reloc
        # slots). The fixed 0x08000000 window makes those words dereference
        # directly; when that window is unavailable and the mirror is
        # re-homed, these words must be patched too or they point at unmapped
        # memory. Re-homing only rewrites words that actually look like
        # 0x08xxxxxx addresses. Pure data blobs (no reloc slots) are excluded
        # so their coincidental 0x08xxxxxx bytes are never rewritten.
        slice_relocs = set()
        for cname, off_in_array, rom_off, size in all_manifest:
            if cname not in all_reloc_owners:
                continue
            for off in range(off_in_array, off_in_array + size, 4):
                slice_relocs.add((cname, off))
        c_lines.append('const unsigned int *const pcSliceRelocTable[] = {')
        for cname, off in sorted(slice_relocs):
            c_lines.append(f'    (unsigned int *)((u8 *){cname} + 0x{off:x}),')
        c_lines.append('};')
        c_lines.append(f'const unsigned pcSliceRelocCount = {len(slice_relocs)};')
