...
void sub_802D940(void)
{
  ...
  {
    case 2:
        if ((gUnknown_203B2F8->unk9 != 0) && (((mail = &gUnknown_203B490->unkF0[gUnknown_203B2F8->unk10]), mail->mailType > 5) && (gUnknown_203B2F8->unkA == mail->dungeon.id)))
        {
            sub_802D1A0(0x7);
        }
        else {
...
bool8 sub_802DAA8(void)
{
  ...
  if (gUnknown_203B2F8->unk9 == 0) {
    ...
    if ( (mail->mailType != WONDER_MAIL_TYPE_THANK_YOU) && (gUnknown_203B2F8->unkA == mail->dungeon.id)) return 0;
    return 1;
  }
}
...
_0802DAE8:
    ...
    for( mail = &gUnknown_203B490->unkF0[0], counter = 0; counter < 8; mail++, counter++)
    {
      ...
      if (((mailType == 0) || (mailType == WONDER_MAIL_TYPE_THANK_YOU)) ||
         ((WONDER_MAIL_TYPE_THANK_YOU < mailType && (gUnknown_203B2F8->unkA != mail->dungeon.id)))) goto _0802DAE8;
    }
    return 0;
  }
}
